...
export default function ExecutionSection({
  ...
  const modules = {
    toolbar: {
      container: `#toolbar-execution-${index}`,
    },
  };
    ...
    <Card className="border border-black/10 hover:border-black/20 transition-all duration-200 bg-white/80 backdrop-blur-sm">
      <CardContent className="p-3 space-y-2">
        <div className="flex justify-between items-center">
          <Label
            htmlFor={`executionSections.${index}.title`}
        ...
        <Field name={`executionSections.${index}.title`}>
          {({ field, meta }: any) => (
            <div>
              <Input
                {...field}
                id={`executionSections.${index}.title`}
              ...
              />
              <ErrorMessage
                name={`executionSections.${index}.title`}
                component="div"
        ...
        <div className="text-editor">
          <Label
            htmlFor={`executionSections.${index}.content`}
            className="font-semibold text-sm text-gray-700"
          >
            Content
          </Label>
          <div className="mt-1">
            <Field name={`executionSections.${index}.content`}>
              {({ field, form, meta }: any) => (
                // <div>
                //   <ReactQuill
                //     value={field.value}
                ...
                //   />
                //   <ErrorMessage
                //     name={`executionSections.${index}.content`}
                //     component="div"
                //     className="text-red-500 text-xs mt-1"
                //   />
                // </div>
                <div>
                  {isLoaded && (
                    <div>
                      <div id={`toolbar-execution-${index}`}>
